<template>
	<view>
		<u-grid :col="6" :border="false">
			<u-grid-item v-for="(item, index) in memberItem" :index="index" :key="item.id" @tap="linkTo(item.id)">
...
<script>
	...
	export default {
		...
		methods:{
			...
			queryMems(){
				this.$socket.queryMembers(this.chatObj.chatId, this.user.operId, res => {
					if (res.success) {
						this.$u.vuex('memberItem',res.members)
						uni.setNavigationBarTitle({
							title: '群成员(' + this.memberItem.length +')'
						});
					} else {
						uni.showModal({
							title: '获取数据失败',
