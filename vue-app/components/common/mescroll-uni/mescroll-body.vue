<template>
	<view class="mescroll-body" :style="{'minHeight':minHeight, 'padding-top': padTop, 'padding-bottom': padBottom}" @touchstart="touchstartEvent" @touchmove="touchmoveEvent" @touchend="touchendEvent" @touchcancel="touchendEvent" >
		<view class="mescroll-body-content" :style="{ transform: translateY, transition: transition }">
			...
			<!-- 空布局 -->
            <slot v-if="isShowEmpty" name="empty">
                <mescroll-empty :option="mescroll.optUp.empty" @emptyclick="emptyClick"></mescroll-empty>
            </slot>
...
<script>
	...
	export default {
		...
		props: {
			...
			height: { // 底部是否偏移TabBar的高度(默认仅在H5端的tab页生效)
				type: [String, Number],
				default: 0
		...
		computed: {
			...
			minHeight(){
				...
				if(height > 0){
					 minHeight = this.toPx(height);
				}else if(height < 0){
					minHeight = this.toPx('100%') + uni.upx2px(height)
				} else {
